...
Vector2 lightPosition(200.f, 0.f); //광원의 위치
LinearColor lightColor;
Vector2 circlePosition; //물체의 위치
...
void SoftRenderer::Render2D()
{
	...
	{
		//원점에서 해당 픽셀 방향으로 향하는 벡터를 구하고 이를 정규화 시켜 벡터 n에 저장
		Vector2 n = (v - circlePosition).GetNormalize(); 

		//해당 픽셀에서 광원으로 향하는 벡터를 구하고 이를 정규화 시켜 벡터 l에 저장
		Vector2 l = (lightPosition - v).GetNormalize();

		//n과 l의 내적 결과를 shading에 저장
